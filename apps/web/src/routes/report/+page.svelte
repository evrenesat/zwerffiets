<script lang="ts">
  ...
  const addPhotos = async (incoming: File[]): Promise<void> => {
    ...
    try {
      const normalizedPhotos = await Promise.all(
        acceptedPhotos.map(
          async (photo) => await normalizePhotoForUpload(photo),
        ),
      );
  ...
  const submit = async (): Promise<void> => {
    ...
    const emailToSubmit =
      !isLoggedIn && reporterEmail.trim() ? reporterEmail.trim() : undefined;
    ...
    try {
      ...
      result = await fetchJson<{ public_id: string; tracking_url: string }>(
        "/api/v1/reports",
        {
          method: "POST",
          body: formData,
      ...
      if (failure.queueOffline) {
        try {
          const queuedPhotos = await Promise.all(
            photos.map((photo) => toDataUrl(photo)),
          );
          enqueueReport({
            location,
            tags: selectedTags,
...
<svelte:head>
  <meta property="og:title" content={t($uiLanguage, "report_title")} />
  <meta property="og:description" content={t($uiLanguage, "app_subtitle")} />
  <meta
    property="og:url"
    content="{PUBLIC_BASE_URL || 'https://zwerffiets.org'}/report"
  />
  <meta
    property="og:image"
...
<div class="report-form-wrap">
  ...
  <div class="field">
    ...
    <textarea
      ...
      rows="4"
      bind:value={note}
      placeholder={t($uiLanguage, "report_note_placeholder") || ""}
    ></textarea>
  </div>
  ...
  <button class="submit" disabled={!submitAllowed()} onclick={submit}>
    ...
    >
    {submitting
      ? t($uiLanguage, "report_submitting")
      : t($uiLanguage, "report_submit")}
  </button>
  ...
  {#if result}
    <div class="success">
      <svg
        ...
        stroke-linejoin="round"
        aria-hidden="true"
        style="flex-shrink:0; color: var(--primary); margin-top: 0.1rem"
      ...
      <div>
        ...
        <a href={result.tracking_url}
          >{t($uiLanguage, "report_success_track")}</a
        >
      </div>
    </div>
  {/if}
